...
import {
  ...
  getSpouseIds, getAvatarUrl, isActiveMarriage, findRelationPath,
} from '@/lib/family';
...
// ─── Static data ──────────────────────────────────────────────────────────────
const RAW_MEMBERS = (familyData as any).members as FamilyMember[];
...
function HomePageInner() {
  ...
  const members = useMemo(() => RAW_MEMBERS, []);
    ...
    return (
      <div className="flex flex-col min-h-screen" style={{ background: 'var(--bg)' }}>
        <header className="border-b px-4 sm:px-6 py-4" style={{
          borderColor: 'var(--border)', background: 'rgba(15,14,23,0.95)', backdropFilter: 'blur(12px)',
        }}>
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden" style={{ background: 'var(--bg)' }}>
      <header className="shrink-0 border-b print:hidden" style={{
        background: 'rgba(15,14,23,0.92)', backdropFilter: 'blur(12px)', borderColor: 'var(--border)',
      }}>
      ...
      {/* Panels */}
      {showBirthdays && (
        <BirthdayPanel members={members} onNavigate={handleNavigate} onClose={() => setShowBirthdays(false)} />
      )}
      {showStats && (
        <StatsPanel members={members} onClose={() => setShowStats(false)} />
      )}
